...
class AbsModule(ABC, Module):
    ...
    def patch(self, x: ndarray, input_patch_size: int, *, pad_mode: str = "symmetric", **kwargs):
        ...
        qh = self.multiple_of - self.patch_slices_remainder(height, out_patch_size)
        qw = self.multiple_of - self.patch_slices_remainder(width, out_patch_size)
    ...
    def patch_slices_remainder(self, length: int, output_patch_size: int):
