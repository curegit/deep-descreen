...
from descreen.training import patch_size, batch_size, num_images
from descreen.training.loss import descreen_loss
from descreen.training.data import HalftonePairDataset, CachedHalftonePairDataset, enumerate_loader
...
def train[
    ...
    if from_cache:
        training_data = CachedHalftonePairDataset(train_data_dir, input_size, padding)
    else:
        training_data = HalftonePairDataset(train_data_dir, profile, input_size, padding, augment=True, debug=True, debug_dir=output_dir, extend=batch_size * 30, save_dir=cache_to).as_tensor()
    ...
    def train_loop(max_samples: int, *, graceful: bool = True):
        ...
        valid_dataloader = DataLoader(valid_data, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=32, persistent_workers=False, pin_memory=True, multiprocessing_context="spawn")
    ...
    def train_step(x, y, i):
        ...
        loss = descreen_loss(pred_mid, y, tv=0.0001, gd=0.1) + 1.5 * descreen_loss(pred_full, y, tv=0.0001, gd=0.1)
        optimizer.zero_grad()
        loss.backward()
        print(f"loss: {loss}")
        optimizer.step()
